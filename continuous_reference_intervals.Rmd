...
```
Instead of manually picking $\lambda$, in the `ps` function above, we provided a sequence of numbers, coded as: `lambda = lseq(0.01,30,l=20)`, which is a logarithmic sequence of numbers from 0.01 to 30 of length 20 (0.01, 0.014, 0.0.19, ..., 21.49, 30). By feeding `ps()` this sequence of numbers, the `gcrq` fitting function will calculate the T vs. age curve for all those $\lambda$ smoothing values and identify the optimum (smoothest curve that still passes closest to the points) using cross-validation. In the cross-validation, the curve is calculated using a subset of data points and the accuracy of the fit of the left out points is assessed iteratively.  Using cross-validation, the optimum $\lambda$ value for T in males was 0.19.
...
SHBG                      & 0.08           & unchanged     & 0.12            & unchanged       
\end{tabular}
\end{table}
